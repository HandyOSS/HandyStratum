...
class Stratum extends EventEmitter {
  ...
  async handleSubmit(conn, msg) {
    ...
    if(hash.compare(OGPoolDiff) > 0 && hash.compare(shareTarget) > 0){
      //share is > block target (OGPoolDiff) and > miner share target
      this.logger.debug(
    ...
    //so far so good, add this share to the db!
    this.sharedb.add(subm.username, difficulty);

    /*
    //leaving this in case it can be handy
    this.logger.debug(
    ...
    */
    ...
    let error;
    /*
    //leaving this in case you want to inspect how mask hash applies:
    this.logger.debug('share hash   %s',share.shareHash().toString('hex'));
    this.logger.debug('block target %s',OGPoolDiff.toString('hex'));
  ...
  async handleAddUser(conn, msg) {
    ...
    //Note: function is not needed for un-registered/anonymous stratum. 
